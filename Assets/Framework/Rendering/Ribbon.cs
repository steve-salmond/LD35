...
public class Ribbon : MonoBehaviour
{
    ...
    /** Whether to orient to line normal. */
    public bool OrientToNormals = false;
	...
	private void UpdateSamples()
	{
        ...
        // Orient to normals if desired.
        if (OrientToNormals && lastIndex >= 0)
        {
            var last = samples[lastIndex];
            var delta = sample.Position - last.Position;
            var normal = Vector3.Cross(delta.normalized, transform.forward);
